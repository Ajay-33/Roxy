...
export type NotificationAnalytics = { count: number; postedCount: number; removedCount: number; hourly: Array<{ hour: number; count: number }>; topApps: Array<{ packageName: string; count: number }> };
export interface NotificationAnalyticsStore { notificationAnalytics(query: { deviceId: string; date: string; limit: number }): Promise<NotificationAnalytics>; }
...
export class PostgresEventStore implements EventStore, UsageTotalsStore, TimelineStore, UsageSummaryStore, NotificationSummaryStore, NotificationAnalyticsStore {
    ...
    async notificationAnalytics(query: { deviceId: string; date: string; limit: number }): Promise<NotificationAnalytics> {
        const filters = [query.deviceId, query.date];
        const totals = await this.pool.query(`SELECT COUNT(*)::integer AS count, COUNT(*) FILTER (WHERE event_type='notification.posted')::integer AS "postedCount", COUNT(*) FILTER (WHERE event_type='notification.removed')::integer AS "removedCount" FROM events WHERE device_id=$1 AND event_type IN ('notification.posted','notification.removed') AND (occurred_at AT TIME ZONE observed_timezone)::date=$2::date`, filters);
        ...
        const topApps = await this.pool.query(`SELECT payload->>'packageName' AS "packageName", COUNT(*)::integer AS count FROM events WHERE device_id=$1 AND event_type IN ('notification.posted','notification.removed') AND (occurred_at AT TIME ZONE observed_timezone)::date=$2::date GROUP BY "packageName" ORDER BY count DESC, "packageName" ASC LIMIT $3`, [...filters, query.limit]);
        return { ...totals.rows[0], count: Number(totals.rows[0].count), postedCount: Number(totals.rows[0].postedCount), removedCount: Number(totals.rows[0].removedCount), hourly: hourly.rows.map((row) => ({ hour: Number(row.hour), count: Number(row.count) })), topApps: topApps.rows.map((row) => ({ packageName: row.packageName, count: Number(row.count) })) };
    }
}
