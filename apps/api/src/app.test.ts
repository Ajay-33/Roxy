...
const notificationAnalyticsStore: EventStore & NotificationAnalyticsStore = { ...store, notificationAnalytics: async (query) => {
    assert.equal(query.deviceId, "synthetic-device"); assert.equal(query.date, "2026-08-19");
    return { count: 3, postedCount: 2, removedCount: 1, hourly: [{ hour: 9, count: 2 }], topApps: [{ packageName: "example.synthetic", count: 3 }] };
} };
...
test("returns authenticated deterministic notification analytics", async () => {
    ...
    assert.equal(response.statusCode, 200); assert.deepEqual(response.json(), { count: 3, postedCount: 2, removedCount: 1, hourly: [{ hour: 9, count: 2 }], topApps: [{ packageName: "example.synthetic", count: 3 }] });
